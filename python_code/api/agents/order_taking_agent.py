...
class OrderTakingAgent():
    ...
    def get_response(self, dialog_messages):
        ...
        last_status_chunk = ""
        had_reco_before = False
        for message_index in range(len(dialog_messages)-1, 0, -1):
            entry = dialog_messages[message_index]
            
            agent_name = entry.get("memory", {}).get("agent", "")
            if entry["role"] == "assistant" and agent_name == "order_taking_agent":
                step_number = entry["memory"]["step number"]
                order_blob = entry["memory"]["order"]
                had_reco_before = entry["memory"]["asked_recommendation_before"]
                last_status_chunk = f"""
        ...
        dialog_messages[-1]['content'] = last_status_chunk + " \n " + dialog_messages[-1]['content']

        input_messages = [{"role": "system", "content": system_prompt}] + dialog_messages        

        raw_model = get_chatbot_response(self.client, self.model_name, input_messages)

        # double check json 
        raw_model = double_check_json_output(self.client, self.model_name, raw_model)

        formatted = self.postprocess(raw_model, dialog_messages, had_reco_before)

        return formatted

    def postprocess(self, model_json, dialog_messages, had_reco_before):
        parsed = json.loads(model_json)

        if type(parsed["order"]) == str:
            parsed["order"] = json.loads(parsed["order"])

        reply_text = parsed['response']
        if not had_reco_before and len(parsed["order"]) > 0:
            reco_suggestion = self.recommendation_agent.get_recommendations_from_order(dialog_messages, parsed['order'])
            reply_text = reco_suggestion['content']
            had_reco_before = True

        packaged = {
            "role": "assistant",
            "content": reply_text,
            "memory": {"agent": "order_taking_agent",
                       "step number": parsed["step number"],
                       "order": parsed["order"],
                       "asked_recommendation_before": had_reco_before
                      }
        }
